    ...
    return props.trigger5 ? (
        <div className='popup' style={{position:'fixed',top:0,left:0,width:'100vw',height:'100vh',zIndex:10000}}>
            <div
                ...
                onClick={() => props.setTrigger5(false)}
            />
            <div className='popup-inner' style={{position:'relative',zIndex:2}} onClick={() => props.setTrigger5(false)}>
                <button className="close-btn custom-close-btn" style={{zIndex:3}} onClick={e => {e.stopPropagation(); props.setTrigger5(false);}} aria-label="Fermer la popup">✕</button>
                {props.children}
                <div className='demo-width project-grid'>
                    {projetsPopup5.map((projet, n) => (
                ...
                {!isMobile && zoomState && projetsPopup5[zoomState.projetIdx] && projetsPopup5[zoomState.projetIdx].images && (
                    <div className="modal-backdrop" style={{zIndex:10001}} onClick={e => {e.stopPropagation(); setZoomState(null);}}>
                        <div className="modal-content" style={{background:'transparent',boxShadow:'none',display:'flex',justifyContent:'center',alignItems:'center'}} onClick={e => e.stopPropagation()}>
                            ...
                            >
                                {projetsPopup5[zoomState.projetIdx].images.map((img, idx) => (
                                    <SwiperSlide key={idx}>
                                        <img
                                            src={img}
                                            alt={projetsPopup5[zoomState.projetIdx].titre + ' zoom photo ' + (idx+1)}
